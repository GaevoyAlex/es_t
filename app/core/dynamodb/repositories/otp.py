...
class OTPRepository(BaseDynamoDBConnector):
    ...
    def delete_old_otps_for_email(self, email: str, otp_type: str) -> int:
        """Удаляет старые OTP коды для определенного емаила"""
    ...
    def get_otp_stats(self, email: str = None) -> Dict[str, Any]:

        if email:

            otps = self.get_otps_by_email(email)
            
            total = len(otps)
            used = len([otp for otp in otps if otp.get('is_used', False)])
            active = len([otp for otp in otps if not otp.get('is_used', False) and 
                         otp.get('expires_at', '') > datetime.utcnow().isoformat()])
            ...
            return {
                ...
                'used_otps': used,
                'active_otps': active,
                'expired_otps': total - used - active
            }
        else:
            all_otps = self.scan_items(self.table_name)
            current_time = datetime.utcnow().isoformat()
            
            total = len(all_otps)
            used = len([otp for otp in all_otps if otp.get('is_used', False)])
            expired = len([otp for otp in all_otps if otp.get('expires_at', '') < current_time])
